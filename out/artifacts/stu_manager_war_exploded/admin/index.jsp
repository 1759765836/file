...
<body>
	...
	<table id="stuTb" width="800" border="1" align="center">
		...
		<tr>		<%--  中括号操作符：适用于特殊字符、变量、复选框 ${requestScope['pageModel']["stuList"].[0] } --%>
				<td>${stu.id }</td>
				<td><a style="text-decoration: none;" href="stuServlet?param=detail&id=${stu.id} "> ${stu.name }</a></td>
				<td>${stu.age }</td>
				<td>${stu.address }</td>
				<td>${stu.classes.name }</td><!-- student对象.getClass().getName() -->

				<td align="center"><a href="stuServlet?param=modify&id=${stu.id }&pageNo=${requestScope.pageModel.pageNo }">修改</a>&nbsp;&nbsp; |
